...
//                           .force('y', d3.force().strength(0.002))
    ...
    function loadAwsmDataFromURL(awsmDataURL){
        nodes = [];
        relationships = [];

        d3.json(awsmDataUrl, function(error, data) {
            if (error) {
                throw error;
            }

            updateWithAwsmData(data);
        });

    }
    function updateWithAwsmData(awsmdata){
        data = convertAwsm2Neo4j(awsmdata);
        updateWithNeo4jData(data);
    }
    function convertAwsm2Neo4j(awsmdata){
        alert(awsmdata.length);
    ...
    function neo4jDataToD3Data(data) {
        ...
        if (data.results){
            ...
            data.Topics.forEach(function(topic){
                ...
                node.id = id;
                node.labels[0] = topic.Highest_prob[0];
                node.properties = topic;
                id++;
                graph.nodes.push(node);
            });
            data.TopicCorr.forEach(function(corr){
                var relationship = {};
